function [Dom, Aom, Answer, Bn,s] = Analysis(stats1, stats2, Line, Img, flag)
    ...
    for i = 1 : length(Answer)
        if ~isempty(Answer(i).Loc)
            ...
            for j = 1 : size(tempi, 1)
                ...
                s=s+1;
                H=[H h];
                text(tempij(1), tempij(2), awij, 'color', 'g');
            end
        end
    end
    save(fullfile(pwd, 'result.mat'), 'H');
    Err = [0 0 0];
    if ~isempty(Bn(1).Loc)
        tempi = Bn(1).Loc;
        resulti = Bn(1).result;
        for j = 1 : size(tempi, 1)
            tempij = tempi(j, :);
            resultij = resulti(j, :);
            text(tempij(1), tempij(2), num2str(resultij), 'color', 'g');
        end
    else
        Err(1) = 1;
    end
